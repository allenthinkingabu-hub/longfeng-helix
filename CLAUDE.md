...
### Rule 6 — Token budgets（项目适配版 · 不要套用原模板）
sub-agent per-task ≤ 200K tokens（实测 Coder ~107K、Tester 最多 168K）。超过先 surface compaction，再决定是否切下一 attempt。原模板的 4K/30K 是错的数量级，**不适用于本项目**。
