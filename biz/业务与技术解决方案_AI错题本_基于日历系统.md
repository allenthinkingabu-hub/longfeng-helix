...
| 决策节点 | 判据 | 命中去向 | 未命中去向 |
|---|---|---|---|
| 1. 合法 JWT | `Authorization` 头解出 `student_id` 且未过期；**且** URL path **不**含 `/s/:token` / `/observer/:code` | 直达 depLink 原目标；如无则 P-HOME | 继续节点 2 |
...
**节点优先级例外（脱敏边界守护）**：URL path 含 `/s/:token` 或 `/observer/:code` 时，**跳过节点 1 直接走节点 2**——分享链 / 观察者邀请码一律以匿名脱敏视图渲染。已登录身份**仅用于**在页面顶部展示「以正式账号查看」升级 CTA，**不**解锁脱敏字段、**不**返回原图、**不**返回 `student_email` / `chat_id` 等 PII。这条规则保护 §2A.3.2 P-SHARED / P-OBSERVER 的脱敏规则不被"持有 JWT 即解锁"绕过。

**回退与降级**：
- 节点 1 **前端本地判**：`bootstrap/resolve-entry.ts` 先用 jose 解 `Authorization` JWT（验签 + exp + iss + aud），命中则直接 `router.replace('/home')` 或 deepLink 原目标，**不调** `/api/session/resolve`。仅在节点 1 未命中（无 JWT / 过期 / 验签失败）时才走后端 resolve 走节点 2/3。这条规则避免冷启动每次多一次 RTT（小程序冷启动 P95 从 ~1.2s 拉到 ~1.6s+）。
- 离线 / `GET /api/session/resolve` 5xx / 超时 ≥ 800 ms 的降级：**有 stale JWT（exp 已过 ≤ 7 d）→ 接受 stale 进 P-HOME（顶部黄条「离线模式 · 数据可能不是最新」）**；**完全无 JWT → 落 P-LANDING**。**禁止白屏 · 禁止默认全部打回 P-LANDING**（已登录用户网络抖一下就被打回访客落地页 = 体验事故）。
...
|---|---|---|---|---|
| **SC-00** | **冷启动 → 登录态决策 → 路由分发**（核心入口） | 所有 SC 的隐性前置；登录态路由唯一权威 | 决策树节点 1 · P00 · P-HOME（节点 2/3 引用 SC-11/13/14） | 3 |
...
### 2B.1a SC-00 · 冷启动 → 登录态决策 → 路由分发

**场景目的**：用户每次打开 App / 小程序 / H5 / 点击深链，前端 `bootstrap/resolve-entry.ts` + 后端 `GET /api/session/resolve` 协作完成「登录态决策树」（§2A.3.1）三层判断，把用户路由到正确落位页（P-HOME / P00 / P-LANDING / P-SHARED / P-WELCOMEBACK）。**这是 SC-01..SC-15 所有场景的隐性前置**，单独编排是为了让 QA 能验、Coder 能拆 feature、TestDesigner 能生成 test-cases.md。

**前置条件**：用户在任意终端（iOS 微信小程序 / Android H5 / Web）冷启动 App，或从外部点击深链；后端 session-service / file-service / anonymous-service 健康；网络可用。

**核心路径编排（节点 1 happy path：已登录直达 P-HOME）**：

| # | 用户动作 | 页面前台 | 后端/事件 | 前端状态 | 埋点 | 耗时预算 |
|---|---|---|---|---|---|---|
| 1 | 点击桌面图标 / 微信小程序入口 / 扫码 | OS launch screen 显示（iOS/Android/微信平台原生 · 200 ms 系统级闪屏 · 不是我们 spec 的页面）| — | (前端尚未挂载) | `boot_launch{platform,entry_source}`（前端 mount 后补上报）| 200 ms（系统级 · App 不可控）|
| 2 | 等待登录态决策（对用户透明）| OS launch screen 仍可见 · 前端 `bootstrap/resolve-entry.ts` mount 后静默 fetch（不渲染任何独立页）| `GET /api/session/resolve` 携带 `Authorization`（若有）+ `device_fp` + URL query → 后端按 §2A.3.1 决策树返回 `{verdict: HOME / LOGIN_REQUIRED / SHARED / WELCOMEBACK / LANDING, redirect_to?}` | (App root) | `boot_resolve_start` | ≤ 300 ms |
| 3 | 后端返回 `verdict=HOME` | `router.replace('/home')` · OS launch screen 自动淡出 → P-HOME 首屏骨架屏 | `GET /api/home/today` 聚合接口预热 | `home.LOADING` | `boot_resolve_done{verdict=HOME,latencyMs}` | ≤ 200 ms |
| 4 | P-HOME 首屏渲染完成 | 今日复习大卡 / 本周条带 / 消息聚合 / 薄弱知识点 全部就绪 | — | `home.READY` | `home_view{from=boot}` | ≤ 1.0 s |

**核心路径编排（节点 1 未命中分支：JWT 不存在/过期 → P00 → redirect）**：

| # | 用户动作 | 页面前台 | 后端/事件 | 前端状态 | 埋点 | 耗时预算 |
|---|---|---|---|---|---|---|
| 1' | 点击桌面图标 / 推送通知（深链 `wb://review/exec?nid=12345`）| OS launch screen 显示（平台原生）| — | (前端尚未挂载) | `boot_launch{deeplink=wb://review/exec}`（前端 mount 后补上报）| 200 ms |
| 2' | 等待决策 | OS launch screen 仍可见 | `GET /api/session/resolve` → 后端发现无 `Authorization` 或 JWT 已过期 → 返回 `{verdict: LOGIN_REQUIRED, redirect_to: 'wb://review/exec?nid=12345'}` | (App root) | — | ≤ 300 ms |
| 3' | 自动跳 P00 登录页 | `router.replace('/login?redirect=...')` · OS launch screen 淡出 → P00 渲染，顶部小字"登录后将自动打开你的复习" | — | `login.IDLE` | `boot_resolve_done{verdict=LOGIN_REQUIRED}` / `login_view{from=boot,redirect=...}` | ≤ 500 ms |
| 4' | 学生输入手机号 / 微信一键登录 | P00 按钮 loading | `POST /api/auth/login` → 返回 JWT | `login.AUTHING` → `login.SUCCESS` | `login_submit{method=wechat}` / `login_success` | ≤ 1.5 s |
| 5' | 登录成功自动 redirect | P00 淡出 → 目标深链页（本例 P08 复习执行）首屏骨架屏 | `GET /api/review/nodes/{nid}` | `login.SUCCESS` → `exec.LOADING` | `login_redirect{target=wb://review/exec}` | ≤ 500 ms |

**节点 2/3 分支（匿名态）**：详见各自 SC 卡，本 SC 不重复编排：

| `verdict` | 路由去向 | 关联 SC |
|---|---|---|
| `SHARED` | P-SHARED | SC-13（§2B.14） |
| `WELCOMEBACK` | P-WELCOMEBACK | SC-14（§2B.15 · P1） |
| `LANDING` | P-LANDING | SC-11（§2B.12） |

**关键断言点（System Invariants）**：

- 节点 1（JWT 验签 + exp 检查）**仅在前端 jose 本地判未命中时**才调 `GET /api/session/resolve`；调用时**必须**是冷启动后第一个业务接口（早于 `/api/home/today` 等）。本地判命中（已登录 + JWT 有效）的 happy path **不打** resolve 接口（按 §2A.3.1「回退与降级」第 1 条）。
- `verdict` 5 种枚举值穷尽决策树所有出口，**不允许返回其他值**（前端 zod / TS 联合类型守护）
- JWT 过期时后端**必须**回 `redirect_to`（原始深链 URL），不丢失用户意图
- `device_fp` 仅用于决策节点 3 + 风控，**不写任何业务表**（合规要求）
- 决策树任一节点抛 5xx 或 OS launch screen 已淡出但 resolve 仍未返回，前端**直接 `router.replace('/welcome')` 落 P-LANDING**而非白屏。**本 SC 不存在"我们自己的中间闪屏页"** —— OS launch screen 是 iOS/Android/微信平台原生 · P-LANDING 是匿名态天然兜底入口（§2A.3.1 节点 2 失效已规定 · 本 SC 扩展到 5xx + 超时）
- **`account_device` P0 写盘 / P1 读盘**：P0 期 SC-01..SC-10 任一登录成功路径（`POST /api/auth/login` / `wechat-login` 返回 200 后）后端**必须 silent upsert** 一行 `account_device`（`device_fp` ↔ `student_id` 软绑定 + `last_login_at` + `last_seen_ua`），**但 P0 期决策树节点 3 始终返回未命中**（按 §2A.3.1 line 293）。P1 启用 P-WELCOMEBACK 时即可读到 P0 期攒下的全量历史，无需经历"上线第一周零识别率"的冷启动期。**违反此条 = P1 回流漏斗第一周白埋点**。

**QA 用例（GIVEN / WHEN / THEN）**：

| TC ID | 类型 | GIVEN | WHEN | THEN |
|---|---|---|---|---|
| TC-00.01 | 正常 | 用户上次登录 JWT 未过期 · 无深链 · 无 shareToken | 冷启动 App | 走「节点 1 happy path」步骤 1-4 · `verdict=HOME` · 屏幕最终落在 P-HOME · 首屏 TTI ≤ 1.5 s · 埋点 `boot_launch` + `boot_resolve_done{verdict=HOME}` + `home_view{from=boot}` 各 1 条 |
| TC-00.02 | 正常 | 用户从未登录或主动登出 · 无深链 | 冷启动 App | 走「节点 1 未命中分支」步骤 1'-3' · `verdict=LOGIN_REQUIRED` · 屏幕最终落在 P00 · 顶部**不**显示 "登录后将自动打开 XXX" 文案（因无 redirect_to） · 埋点 `boot_resolve_done{verdict=LOGIN_REQUIRED}` + `login_view{from=boot,redirect=null}` |
| TC-00.03 | 异常 | 用户上次登录 JWT 已过期 · 从推送点击深链 `wb://review/exec?nid=12345` | 冷启动 App | 走「节点 1 未命中分支」步骤 1'-5' · `verdict=LOGIN_REQUIRED` · `redirect_to=wb://review/exec?nid=12345` · P00 顶部小字显示"登录后将自动打开你的复习" · 学生登录成功后**自动**落到 P08 nid=12345（不经 P-HOME 中转） · 埋点 `login_redirect{target=wb://review/exec?nid=12345}` 1 条 |

> 说明：本 SC 故意**不**写 TC 覆盖 verdict=SHARED / WELCOMEBACK / LANDING 三个分支 —— 它们的 QA 用例已分别归属 SC-13 / SC-14 / SC-11，避免重复维护漂移。SC-00 只保证「决策树本身被正确路由」，分支落地页的业务由各 SC 自治。
...
**前置条件**：前置 SC：SC-00 路由分发已落 P-HOME（学生已登录态）；网络良好；相机权限已授予；后端各服务健康。
...
**前置条件**：前置 SC：SC-00 路由分发已落 P-HOME（学生已登录态）；系统已推送 `T3 D7` 到期消息到手机；推送点击目标 `wb://review/exec/nid=12345`；学生有连续 3 题待复习。
...
**前置条件**：前置 SC：SC-00 路由分发已落 P-HOME（学生已登录态）；今日共 8 题 · 已完成 0 · 网络稳定。
...
**前置条件**：前置 SC：SC-00 路由分发已落 P-HOME（学生已登录态）；学生在 P08 执行 T3 节点（nid=500）· qid=200 的 plan 状态：T0/T1/T2 GRADED·MASTERED，T3 OPEN，T4/T5/T6 SCHEDULED · 日历存在 7 条 relation_id=question:200:node:* 的 event（3 COMPLETED + 4 SCHEDULED）。
...
**前置条件**：前置 SC：SC-00 路由分发已落 P-HOME（学生已登录态）；今日是 4 月 21 日 · 4 月 28 日有 T4 复习节点 `nid=700` qid=200 · 日历筛选"显示复习"默认为"开"。
...
**前置条件**：前置 SC：SC-00 路由分发已落 P-HOME（学生已登录态）；学生在 P10 · 4/21（周二）· 17:30 有"接奶奶放学"家庭事件 `eventId=E900 · relation_type=FAMILY · relation_id=family:member:grandma`。
...
**前置条件**：前置 SC：SC-00 路由分发已落 P-HOME（学生已登录态）；学生完成 SC-01 步骤 1-4（成功上传），进入步骤 5 (P03)；后端 mock 两个模型均超时。
...
**前置条件**：前置 SC：SC-00 路由分发已落 P-HOME（学生已登录态）；学生之前偏好 `Asia/Shanghai (UTC+8)`，当前通过 VPN 或出国在 `America/Los_Angeles (UTC-7/-8)` 登录。
...
**前置条件**：前置 SC：SC-00 路由分发已落 P-HOME（学生已登录态）；学生已绑定家长账号（通过 P13 "家长绑定"）；家长在家长端已登录。
...
**前置条件**：前置 SC：SC-00 路由分发已落 P-HOME（学生已登录态）；学生在 P06 查看 qid=200 · plan 状态：T0-T3 GRADED·MASTERED，T4-T6 SCHEDULED · 日历 7 条 event（3 COMPLETED + 3 SCHEDULED + 1 被当前）。
...
### 15.5 与 satellite biz docs 对照表

> 本表由 `.harness/skills/gen-biz-doc.md` skill 在落地 satellite 时自动追加 · 每行登记 1 份 satellite biz doc · 防止 satellite 孤立。
