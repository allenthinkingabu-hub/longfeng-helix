# SC01-T02 Tester 工作日志 · team-1 · attempt-1
...
| # | 测试用例 | 覆盖 | 结果 | 耗时 |
|---|---------|------|------|------|
| 1 | AC1-3 happy path · P02→P03→SSE→P04 | AC1 跳转, AC2 骨架屏, AC3 SSE taskId | PASS | 1.7s |
| 2 | AC4 createPending 5xx → ERROR banner | AC4 留 P02 | PASS | 1.3s |
| 3 | AC5 analyze-by-url 5xx → ERROR banner | AC5 留 P02 | PASS | 1.3s |
| 4 | AC6 SSE failure → P03 + fallback banner | AC6 不阻塞 + banner | PASS | 9.3s |
| 5 | TI1 X-Idempotency-Key | TI1 幂等键 | PASS | 1.3s |
| 6 | TI2 analyze-by-url body | TI2 请求体 | PASS | 1.3s |

## testcase 数量

6 个 testcase passed（与 JUnit XML `<testsuites tests="6" failures="0">` 一致）
