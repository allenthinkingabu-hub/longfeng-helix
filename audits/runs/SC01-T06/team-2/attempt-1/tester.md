# SC-01-T06 Tester 验收日志 · attempt-1

## 任务摘要
- Task: SC01-T06 · question.created MQ → review-plan 7 nodes → calendar-core 7 events
- Branch: `claude/sc01-t06-backend-review-plan`
- Coder commit: `8aff994`

## DoR 检查
- `dor_c1_to_c6_required: false` — DoR C1-C6 物理验证不强制要求
- Coder 交付物齐全: `coder.md` + `bugs-found.md` + `test-reports/e2e/coder/backend-it/verify.log` + XML
- E2E IT 脚本存在: `T06QuestionCreatedE2EIT.java`
...
| `ReviewPlanService.NODE_OFFSETS` [2h,1d,2d,4d,7d,14d,30d] | AC3 EXPECTED_OFFSETS 常量 | MATCH |
...
| `CalendarBatchCreateService.recover()` 写 outbox | AC5 outbox row pending → relay dispatched | MATCH |
| `ReviewPlan.STATUS_ACTIVE=0` + strategyCode=EBBINGHAUS_SM2 + easeFactor=2.5 | AC2 DB 行断言 | MATCH |
| `CalendarOutboxRelayJob.execute()` txTemplate | AC5 relay dispatched | MATCH |

### Mock 审计
- Mock 数量: 1 个 (`T06TogglableCalendarStub` 替代 `CalendarFeignClient`)
- 合理性: calendar-core 是外部 Feign 目标，stub 是必要的 (不可能在 IT 里启动 calendar-core 微服务)
- Mock 计数: 1 ≤ 5 阈值

## 独立验证

### Tester 独立跑 (Round 1 — Coder 原始代码)
```
...
### Tester 追加断言后 (Round 2 — 含 retry count 断言)
```
cd backend/review-plan-service
mvn verify -Dsurefire.skip=true -Dfailsafe.includes="**/T06QuestionCreatedE2EIT.java"
Tests run: 5, Failures: 0, Errors: 0, Skipped: 0
...
## 结论
- 5 testcase 全部通过，覆盖 AC1-AC6 + TI1-TI5
