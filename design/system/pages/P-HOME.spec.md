...
|---|---|---|---|---|---|---|---|
| 1 | GET | `/api/home/today?tz=Asia/Shanghai` | `X-User-Id`, 可选 `X-User-Timezone` | — | `{tz, today:{total, done, circleProgress}, resume:null}` (MVP 子集 · A05 §2.4) | ≤ 400ms (biz §2A.3.3 性能预算) | 服务降级 → 顶部黄条 "部分数据正在同步" + 允许拉下刷新 |
...
`streak / weekSparkline / weekStrip / messages / weakKP / quickEntries / studentName / estMin / subjectDist` 这 9 类字段在 MVP 不下发，前端使用本地占位 / hardcoded 渲染。Phase 1 拆独立 module 后扩展 GET `/api/home/today` 响应体（向后兼容 · MVP 字段不变）。
